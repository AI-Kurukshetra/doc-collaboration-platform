  ...
  const handleSignup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (roleError || !roleRow) {
        throw new Error("Selected role not found. Please try again.");
      }
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-900 px-4 py-12 text-slate-100">
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-white/10 bg-white/10 p-8 shadow-2xl shadow-slate-950/40 backdrop-blur">
        ...
        <p className="mt-4 text-sm text-slate-200/80">
          Join your classroom and start collaborating.
        </p>
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSignup}>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <GraduationCap className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-300" />
              <select
                className="w-full appearance-none rounded-xl border border-white/10 bg-slate-900/70 py-2.5 pl-10 pr-9 text-sm text-slate-100 outline-none ring-indigo-400/40 focus:border-indigo-300/50 focus:ring-2"
                value={role}
                onChange={(event) => setRole(event.target.value as RoleOption)}
              >
                <option className="bg-white text-slate-900" value="teacher">
                  Teacher
                </option>
                <option className="bg-white text-slate-900" value="student">
                  Student
                </option>
              </select>
            </div>
